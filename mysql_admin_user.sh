...
mysql -e "DROP USER IF EXISTS 3uauth@localhost;"
...
mysql -e "CREATE USER 3uauth@localhost IDENTIFIED WITH caching_sha2_password BY '$(tr -dc 'A-Za-z0-9' < /dev/urandom | head -c 41)';"
mysql -e "REVOKE ALL PRIVILEGES ON *.* FROM ${APP_NAME}_def@localhost;"
mysql -e "REVOKE ALL PRIVILEGES ON *.* FROM 3uauth@localhost;"
